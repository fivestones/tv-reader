...
#trying out the sg.Window:
window = sg.Window('this is the title', layout, no_titlebar=True, location=(0,0), keep_on_top=True)
while True:
...
def is_Next(btn):
    return btn.startswith("Next") or btn == "MouseWheel:Down" or btn.startswith("Right")


def is_Prior(btn):
    return btn.startswith("Prior") or btn == "MouseWheel:Up" or btn.startswith("Left")
...
while True:
    #load the next page
    btn, value = form.Read() #this is where the new page is drawn
